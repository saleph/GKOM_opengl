...
void Shader::set3FloatUniform(const std::string &name, float val1, float val2, float val3) const {
	use();
	unsigned transformLoc = glGetUniformLocation(shaderProgramId, name.c_str());
	glUniform3f(transformLoc, val1, val2, val3);
}

void Shader::set3FloatUniform(const std::string &name, const glm::vec3 &vec) const {
	use();
	unsigned transformLoc = glGetUniformLocation(shaderProgramId, name.c_str());
